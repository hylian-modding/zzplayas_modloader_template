...
export default class ootscript implements IModelScript{
    ...
    tunicRefs: Map<Age, Array<IModelReference>> = new Map<Age, Array<IModelReference>>();
    ...
    onTunicChanged(ref: IModelReference, tunic: Tunic): IModelReference {
        ...
        let age: Age = this.core.save.age;
        switch(age){
            case Age.ADULT:
                ...
                break;
            case Age.CHILD:
                if (this.tunicRefs.get(Age.CHILD)!.length > 0){
                    let newRef = this.tunicRefs.get(Age.CHILD)![tunic];
                    newRef.script = this;
                    return newRef;
                }
                break;
